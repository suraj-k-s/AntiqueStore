...
<html>
    <head>
        ...
        <%
            ...
            if (request.getParameter("btn_checkout") != null) {
                ...
                String selC = "select * from tbl_carthead  ch inner join tbl_cartbody cb on cb.cart_id=ch.cart_id where ch.customer_id='" + session.getAttribute("customerid") + "'and ch.booking_status='0'";
                ResultSet rsC = con.selectCommand(selC);
                if (rsC.next()) {
                    String sel = "select * from tbl_productstock where product_id='" + rsC.getString("product_id") + "'";
                    ResultSet rsdd = con.selectCommand(sel);
                    while (rsdd.next()) {
                        int current = Integer.parseInt(rsdd.getString("pstock_qty"));
                        current = current - Integer.parseInt(rsC.getString("cart_qty"));
                        String upds = "update tbl_productstock set pstock_qty='" + current + "' where product_id='" + rsC.getString("product_id") + "'";
    ...
    <body onload="recalculateCart()">
        ...
        <form method="post">
            ...
            <%                String sel = "select * from tbl_carthead b inner join tbl_cartbody c on c.cart_id=b.cart_id where b.customer_id='" + session.getAttribute("customerid") + "' and booking_status='0'";
                System.out.println(sel);
                ResultSet rs = con.selectCommand(sel);
                while (rs.next()) {
                    String selprd = "select * from tbl_product  where product_id='" + rs.getString("product_id") + "'";
                    ResultSet rsprd = con.selectCommand(selprd);
                    if (rsprd.next()) {
                        String selprdStock = "select sum(pstock_qty) as stock from tbl_productstock where product_id='" + rsprd.getString("product_id") + "'";
            ...
            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assets/Productphoto//<%=rsprd.getString("product_image")%>"
                        />
                </div>
                ...
                <div class="product-price"><%=rsprd.getString("product_rate")%></div>
                ...
                <div class="product-line-price">
                    <%
                        float pr = Float.parseFloat(rsprd.getString("product_rate"));
