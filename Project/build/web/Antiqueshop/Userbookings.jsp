    ...
    <body>
        <%
            ...
            if (request.getParameter("aid") != null) {
                String upQry = "update tbl_carthead set booking_status='" + request.getParameter("sts") + "' where cart_id='" + request.getParameter("aid") + "'";
                if (con.executeCommand(upQry)) {
                    response.sendRedirect("Assign.jsp?cid=" + request.getParameter("aid"));
                }
            }

            String selQ = "select DISTINCT(ch.cart_id) as id from tbl_carthead ch inner join tbl_cartbody cb on cb.cart_id=ch.cart_id inner join tbl_product p on p.product_id=cb.product_id where shop_id='" + session.getAttribute("shopid") + "'";
